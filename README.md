...
💡 Ключевая концепция:
Благодаря Docker-сети monitoring_net мы можем обращаться к контейнерам по их именам. node_exporter:9100 автоматически разрешится во внутренний IP контейнера.
...
{job="varlogs"} | detected_level = "error" → найти логи, у которых уровень error.

6. Alertmanager + Telegram (Алерты)
6.1. Регистрация Telegram-бота
В Telegram у @BotFather создать нового бота (/newbot) и получить токен.

Найти своего бота в поиске и отправить ему любое сообщение.

На сервере выполнить curl "https://api.telegram.org/bot<ТОКЕН>/getUpdates" и из ответа скопировать свой chat_id.
...
📖 Глоссарий
Target (цель): Сервис, у которого Prometheus собирает метрики.
...
Alert Rule: Условие в Prometheus, при котором срабатывает алерт.
