...
```
_Теперь можно заходить без пароля: `ssh gleb@<IP>`_

### 1.3. Финализация защиты SSH (`/etc/ssh/sshd_config`)

Мы запретили вход по паролю, вход для `root`, ограничили список пользователей и сменили порт на `2224`.
...
> **✅ Решение:** В папке `/etc/ssh/sshd_config.d/` файл хостера (`40-hosting.conf`) содержал `PasswordAuthentication yes` и переопределял наш конфиг. Исправили на `no`.

### 1.4. Быстрый вход по SSH (на локальном ПК)

Создали файл `~\.ssh\config`:
...
Docker упаковывает приложения в изолированные «контейнеры». Мы добавили официальный репозиторий с GPG-ключом.
...
### 2.3. Инфраструктура для мониторинга
...
> **Ключевые концепции:**  
> **`volumes`** — сохраняют данные на диске хоста при удалении контейнера.  
> **`ports`** — пробрасывают порты из контейнера на хост.  
> **`user: "1000:1000"`** — процесс в контейнере работает от пользователя `gleb`.
...
- `{job="varlogs"} |= "error"` — найти логи со словом `error`.
    
- `{job="varlogs"} | detected_level = "error"` — найти логи с уровнем `error`.
...
1. В Telegram у `@BotFather` создать бота (`/newbot`) и получить **токен**.
    
2. Отправить боту любое сообщение.
    
3. Выполнить `curl "https://api.telegram.org/bot<ТОКЕН>/getUpdates"` и скопировать **chat_id**.
    ...
    rules:
      - alert: HighCPUUsage
        ...
        annotations:
          ...
          description: "CPU загрузка > 80% (текущая: {{ $value }}%)"
      - alert: HighMemoryUsage
        ...
        annotations:
          ...
          description: "Память занята > 85% (текущая: {{ $value }}%)"

---

## 7. Глоссарий и чек-лист для собеседования
...
- **Volume** — механизм Docker для хранения данных на диске хоста.
    
- **LogQL** — язык запросов к логам в Loki.
    
- **Alert Rule** — условие в Prometheus для срабатывания алерта.