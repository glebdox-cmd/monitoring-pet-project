...
## 8. Автоматизация управления инфраструктурой (Ansible + Git)

### Зачем это нужно
Ручная настройка сервера — это долго и невоспроизводимо. Ansible и Git позволяют применить практику «Infrastructure as Code» (IaC). Вся конфигурация стека мониторинга хранится в GitHub-репозитории и может быть развёрнута одной командой на любом сервере.

### Что сделано
- Все конфигурационные файлы (Prometheus, Grafana, Loki, Alertmanager) и `docker-compose.yml` хранятся в приватном репозитории `monitoring-pet-project`.
- Данные (базы данных, WAL-логи, метрики) исключены из Git и создаются автоматически при запуске.
- Создан Ansible-playbook `deploy_from_github.yml`, который:
  1. Устанавливает Docker и Git.
  2. Клонирует репозиторий с конфигами.
  3. Запускает весь стек через `docker compose`.
  4. Проверяет, что все критические сервисы в статусе `Up`.

### Как развернуть стек с нуля
```bash
ansible-playbook -i hosts deploy_from_github.yml --ask-become-pass
